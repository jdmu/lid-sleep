...
When enabled, it takes a low-level `systemd-inhibit` lock on the lid switch so your system continues running while the screen locks normally.
...
- **Short Tooltips:** `"Prevent Lid Sleep: Active"` and `"Prevent Lid Sleep: Inactive"`.
...
```bash
omarchy plugin add https://github.com/<your-username>/prevent-lid-sleep.git --enable
```
...
cp -r prevent-lid-sleep ~/.config/omarchy/plugins/
omarchy-shell shell rescanPlugins
omarchy-shell shell enablePlugin prevent-lid-sleep '{}'
```
